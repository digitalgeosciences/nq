  ...
  head: () => ({
    meta: [
      { title: "Naif Alqubalee | Backend Software Engineer" },
      {
        name: "description",
        content:
          "Backend Software Engineer specializing in secure, scalable NestJS, Node.js, TypeScript, and PostgreSQL systems.",
      },
      { property: "og:title", content: "Naif Alqubalee | Backend Software Engineer" },
      {
        property: "og:description",
        content:
          "Explore backend projects, skills, and certifications focused on secure, scalable systems.",
      },
    ...
    <>
          ...
          <Dialog open={avatarModalOpen} onOpenChange={setAvatarModalOpen}>
            <DialogTrigger asChild>
              <button
                ...
                aria-label={`View ${data.person.name}'s profile card`}
            ...
            <DialogContent className="profile-modal-content">
              <DialogHeader className="profile-modal-header">
                ...
                <DialogTitle className="profile-modal-name">{data.person.name}</DialogTitle>
                <DialogDescription className="profile-modal-role">
                  {data.person.role} ·{" "}
                  <span className="profile-modal-location">
              ...
              <p className="profile-modal-bio">{data.person.bio}</p>
              ...
              <div className="profile-modal-actions">
                <Button
                  ...
                  onClick={() => {
                    navigator.clipboard.writeText(data.person.email);
                    setCopied(true);
                ...
                >
                  <a
                    href="https://www.linkedin.com/in/naifalqubalee"
                    target="_blank"
          ...
          <div className="shell hero-grid">
            <div className="hero-copy">
              ...
              <h1>
                ...
                <em>building secure, scalable, and maintainable systems.</em>
              </h1>
              <p className="hero-subtitle">
                Specializing in <strong>NestJS</strong>, <strong>Node.js</strong>,{" "}
              ...
              <div className="hero-facts">
                ...
                <div>
                  <span>Location</span>
                  <strong>{data.person.location}</strong>
                </div>
              </div>
            </div>
            <CodePanel />
                ...
                {data.principles.map((principle) => (
                  <div className="principle" key={principle.label}>
                    <span className="principle-icon">
                      <IconByName name={principle.icon} />
                    </span>
          ...
          <div className="shell">
            ...
            <div className="projects-grid">
              {data.projects.map((project) => (
                <Reveal
                  className={`project-card project-card-${(data.projects.indexOf(project) % 4) + 1}`}
                  key={project.title}
                >
      ...
      <footer>
        <div className="shell footer-inner">
          <p>© 2026 {data.person.copyright}</p>
          <a
            href="#top"
...
function TechMark({ name }: { name: string }) {
  ...
  return (
    <span className={`tech-mark tech-${key}`} aria-hidden="true">
      <Mark className="tech-logo" />
    </span>
  );
}
...
function CredentialMark({ name }: { name: string }) {
  ...
  if (Mark) {
    return (
      <span className={`cert-icon cert-${name.toLowerCase()}`} aria-hidden="true">
        <Mark className="cert-logo" />
      </span>
    );
  }
  ...
  return (
    <span className={`cert-icon cert-${name.toLowerCase()} cert-lettermark`} aria-hidden="true">
      {initials}
    </span>
  );
}
...
function CodePanel() {
  ...
  const lines = [
    <>
      <span className="code-decorator">@Controller</span>
      <span>('projects')</span>
    </>,
    <>
      <span className="code-keyword">export class</span>
      <span className="code-type"> ProjectsController </span>
      <span>{"{"}</span>
    </>,
    <>
    ...
    <>
      <span className="code-keyword"> private readonly</span>
      <span> service: </span>
      <span className="code-type">ProjectsService</span>
      <span>,</span>
    </>,
    <>
      <span> ) {"{}"}</span>
    ...
    <>
      <span className="code-decorator"> @Get</span>
      <span>()</span>
    </>,
    <>
      <span className="code-keyword"> async</span>
      <span className="code-function"> findAll</span>
      <span>() {"{"}</span>
    </>,
    <>
  ...
  return (
    ...
    >
      <div className="code-toolbar">
        ...
        <span>projects.controller.ts</span>
        ...
        <code>
          {lines.map((line, index) => (
            <motion.span
              ...
              animate={{ opacity: 1 }}
              transition={{
                delay: reduced ? 0 : 0.15 + index * 0.055,
    ...
    <section id="contact" className="section contact-section" aria-labelledby="contact-title">
      <div className="shell">
        <Reveal className="contact-intro">
          ...
          <p>
            Have a backend challenge, a role, or an idea worth exploring? I’m always open to a
              ...
              <a
                ...
                aria-label={
                  social.url.startsWith("http")
                    ? `Visit ${social.label} profile`
                    : `Email ${data.person.name}`
                }
              >
                <span className="social-icon">
                  <IconByName name={social.icon} />
                </span>
                <span>
                  <small>Connect via</small>
                  {social.label}
                </span>
                <ArrowUpRight />
              </a>
